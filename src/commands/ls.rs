...
pub fn display_files(parent: String, files: Vec<String>, cc: bool, dash: bool) {
    ...
    let max_possible_columns = (terminal_width / 3).max(1); // Minimum 3 chars per column
...
pub fn color(a: String, path: &PathBuf, target: bool) -> String {
    let Ok(link) = path.symlink_metadata() else {
        return format!("\x1b[30;41m{}\x1b[0m", a);
    };
        ...
        match path.metadata() {
            Ok(m) => {
                if target { 
                    ...
                    return format!("\x1b[1;36m{}\x1b[0m", a);
                }
            },
            Err(_) => {
                return format!("\x1b[30;41m{}\x1b[0m", a);
    ...
    if file_type.is_dir() {
        if mode & 0o1000 != 0 {
            return format!("\x1b[30;42m{}\x1b[0m", a);
        } else if mode & 0o2000 != 0 {
            return format!("\x1b[30;43m{}\x1b[0m", a);
        } else if (mode & 0o002) != 0 && (mode & 0o1000) == 0 {
            return format!("\x1b[34;42m{}\x1b[0m", a);
    ...
    if file_type.is_file() {
        if mode & 0o4000 != 0 {
            return format!("\x1b[37;41m{}\x1b[0m", a);
        } else if mode & 0o2000 != 0 {
            return format!("\x1b[30;43m{}\x1b[0m", a);
